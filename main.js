...
async function startHaruka() {
	...
	sock.ev.on('messages.upsert', async chatUpdate => {
      ...
      // cek teks "Awali pagimu dengan bismillah" (ignore huruf besar/kecil)
      if (m.isGroup && /awali pagimu dengan bismillah/i.test(body)) {
          let metadata = await sock.groupMetadata(m.chat)
          let participants = metadata.participants.map(v => v.id)
      
         await sock.sendMessage(m.chat, {
            text: ` Asalamualaikum Warohmatullahi Wabarakatuh Selamat Pagi Smwch,🥰 Alhamdulihah All Semoga Hari Ini Penuh Dengan Keberkahan🥰`,
            mentions: participants
         }, { quoted: m });
      }
			// === AUTOJOIN (dipindah ke dalam messages.upsert) ===
        if (typeof global._lastJoinTs === 'undefined') global._lastJoinTs = 0;
        const getBodyLocal = (msg) => (
        ...
        const OWNER_JID = (global.creator && global.creator.includes('@s.whatsapp.net')) ? global.creator : (global.ownerNumber + '@s.whatsapp.net');
        if (global.autoJoin) {
        (async () => {
          try {
            const body = getBodyLocal(mek) || getBodyLocal(m) || "";
            if (body) {
              const regex = /https:\/\/chat\.whatsapp\.com\/([0-9A-Za-z]+)/g;
              const codes = [...body.matchAll(regex)].map(x => x[1]);
              for (const code of codes) {
                if (Date.now() - (global._lastJoinTs || 0) < (global.joinDelay || 0)) {
                  console.log('[AUTOJOIN] ⏳ Cooldown aktif, skip link.');
                  break;
                }
                ...
                try {
                  ...
                  console.log('[AUTOJOIN] Link invalid/expired:', code, e?.message || e);
                  if (global.autoJoinNotify) {
                    try { 
                      await sock.sendMessage(OWNER_JID, { text: `❌ Link grup invalid/expired:\n${code}` }); 
                    } catch {} 
                  }
                  continue;
                }
      
                const name = info?.subject || 'Tanpa Nama';
                const members = info?.size ?? 0;
      
                if (members < (global.minMember || 0)) {
                  console.log(`[AUTOJOIN] ❌ Skip: ${name} (${members} < ${global.minMember})`);
                  if (global.autoJoinNotify) {
                    try {
                      await sock.sendMessage(OWNER_JID, { 
                        text: `❌ Skip join: *${name}*\n👥 Member: ${members} (min ${global.minMember})` 
                      });
                    } catch {}
                  }
                  continue;
                ...
                try {
                  ...
                  global._lastJoinTs = Date.now();
                  console.log(`[AUTOJOIN] ✅ Joined: ${name} (${members})`);
                  if (global.autoJoinNotify) {
                    try { 
                      await sock.sendMessage(OWNER_JID, { 
                        text: `✅ Berhasil autojoin:\n*${name}*\n👥 Member: ${members}\n⌛ Cooldown: ${Math.round((global.joinDelay||0)/60000)} menit` 
                      }); 
                    } catch {} 
                  }
                  break;
                } catch (e) {
                  console.log('[AUTOJOIN] ⚠️ Gagal join:', name, e?.message || e);
                  if (global.autoJoinNotify) {
                    try { 
                      await sock.sendMessage(OWNER_JID, { 
                        text: `⚠️ Gagal join: *${name}*\nAlasan: ${e?.message || e}` 
                      }); 
                    } catch {} 
                  }
                }
            ...
            console.log('[AUTOJOIN] Fatal:', err);
            if (global.autoJoinNotify) {
              try { await sock.sendMessage(OWNER_JID, { text: `⚠️ Autojoin error:\n${err?.message || err}` }); } catch {}
            }
          }
        })();
      }
...
// helper: ambil teks/caption dari berbagai tipe pesan
const getBody = (msg) => (
  msg?.message?.conversation ||
  msg?.message?.extendedTextMessage?.text ||
  msg?.message?.imageMessage?.caption ||
...
// helper: JID owner
const OWNER_JID = (global.creator && global.creator.includes('@s.whatsapp.net'))
  ? global.creator
