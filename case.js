...
module.exports = sock = async (sock, m, msg, chatUpdate, store) => {
	try {
		...
		const own2Number = '6282278687542'
		...
		if (budy && !m.isNewsletter) {
			...
			if (!m.isGroup && !isCmd && !m.key.fromMe) {
				...
				if (lastInteraction > 21600000) {
					db.data.users[m.sender].pctime = new Date().getTime();
					let welcomeText = `Halo Kak ${pushname}! 👋\n\nMohon diperhatikan! Nomor ini sekarang digunakan sebagai bot dan tidak lagi dikelola langsung oleh owner. Jika Kakak memiliki keperluan atau ingin menghubungi owner, silakan hubungi nomor terbaru berikut:\n\n📞 *wa.me/${ownerNumber}*\n\nBot ini siap membantu dengan berbagai fitur yang tersedia. Silakan pilih menu di bawah ini untuk memulai! Terima kasih atas perhatiannya. 😊`;
					// Auto respon khusus untuk ucapan temanmu di group
      ...
      case 'jpm2': {
          if (!isCreator) return newReply(mess.owner);
          if (!text) return newReply(`⚙️ *Penggunaan:*\n${prefix+command} teks|jeda\n\nContoh: ${prefix+command} Coba config gratis|10000`);
          await newReply(`⏳ *Sedang diproses JPM2...*`);
          const contextInfoFull = {
                forwardingScore: 999,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                ...
                externalAdReply: {
                  ...
                  body: global.botName,
                  thumbnailUrl: global.imageUrl,  // pastikan ada variabel imageUrl 
                  sourceUrl: global.wagc,         // link saluran/wa gc
                  mediaType: 1,
                  renderLargerThumbnail: true
                }
              };
          ...
          // Ambil semua file .hc di folder configs/
          const fs = require("fs");
          let configFiles = fs.readdirSync("./configs").filter(f => f.endsWith(".hc"));
          if (!configFiles.length) return newReply("⚠️ Tidak ada file .hc di folder ./configs");
          for (let group of anu) {
              try {
                  // kirim text utama JPM2
                  let sendMsg;
                  if (/image/.test(mime)) {
                      let media = await sock.downloadAndSaveMediaMessage(quoted);
                      sendMsg = await sock.sendMessage(group, { 
                          image: { url: media },
                      ...
                      sendMsg = await sock.sendMessage(group, { 
                          text: text.split('|')[0],
                          contextInfo: contextInfoFull
                      });
                  }
      
                  // kirim semua config .hc satu per satu, reply ke sendMsg
                  for (let file of configFiles) {
                      await sock.sendMessage(group, {
                          document: { url: `./configs/${file}` },
                          mimetype: "application/octet-stream",
                          fileName: file
                      }, { quoted: sendMsg });
                  }
      
                  // jeda sebelum lanjut ke grup berikutnya
                  await sleep(text.split('|')[1]);
      
              } catch (err) {
                  console.log(`[JPM2] Gagal kirim ke ${group}:`, err);
              }
          }
      
          newReply(`✅ *JPM2 selesai!* Semua grup sudah menerima text & config 🚀`);
      }
      break;
			...
			break;
			case 'savemember': case 'svmem': {
    if (!m.isGroup) return newReply(mess.group);
    if (!(m.isAdmin || isCreator)) return newReply(mess.owner);
    ...
    for (let participant of cmiggc.participants) {
        // Ekstrak nomor telepon dari ID (format: "6281234567890@s.whatsapp.net")
        let phoneNumber = participant.id.split('@')[0];
        
        // Validasi nomor (hapus karakter non-digit)
        phoneNumber = phoneNumber.replace(/\D/g, '');

        // Pastikan nomor tidak kosong dan valid
        if (phoneNumber.length < 5 || phoneNumber.length > 15) continue;

        vcard += `BEGIN:VCARD\nVERSION:3.0\nFN:[${noPort++}] ${phoneNumber}\nTEL;type=CELL;type=VOICE;waid=${phoneNumber}:+${phoneNumber}\nEND:VCARD\n`;
    }

    let nmfilect = './contacts.vcf';
    newReply('Menyimpan ' + cmiggc.participants.length + ' kontak...');
    require('fs').writeFileSync(nmfilect, vcard.trim());
    await sleep(2000);
...
break;
      ...
      case 'standoff': {
        let metadata = await sock.groupMetadata(m.chat)
         ...
         await sock.sendMessage(m.chat, {
            text: ` Asalamualaikum Warohmatullahi Wabarakatuh MARI LOJIN`,
...
break
      ...
      try {
        await m.react('⏱️');
        let userData = db.data.users[m.sender] || {};
        let simplemenu = `📢 MAOUVPN STORE – CONFIG & VPN PREMIUM\n\n`;
        simplemenu += `╭────────────────╮\n`;
        simplemenu += `💎 MAOUVPN OFFICIAL 💎\n`;
        simplemenu += `╰────────────────╯\n\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n`;
        simplemenu += `*📲 SALURAN CONFIG*\n`;
        simplemenu += `➥ https://whatsapp.com/channel/0029VaiKMO3KrWR1A35DRN1O\n`;
        simplemenu += `𝗧𝗘𝗦𝗧𝗜𝗠𝗢𝗡𝗜\n`;
        simplemenu += `➥ https://whatsapp.com/channel/0029Vb6hmd76LwHbky6SbJ2s\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n`;
        simplemenu += `⚡ Config Premium Server Singapura Untuk Injektor Kuota.\n`;
        simplemenu += `✅ FULL GARANSI → Jika EROR langsung ditangani sampai connect!\n\n`;
        simplemenu += `🌐 Online Order/Trial:\n`;
        simplemenu += `https://list.maourafavpn.my.id\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
        simplemenu += `🇸🇬 LIST SERVER VPN\n\n`;
        simplemenu += `📝 SG-DO (2 IP / 200GB Bandwidth)\n`;
        simplemenu += `🚀 CPU DO-Standar-AMD (2) @ 1.49GHz\n`;
        simplemenu += `├ 3 Hari : Rp1.000\n`;
        simplemenu += `├ 7 Hari : Rp3.000\n`;
        simplemenu += `├ 15 Hari : Rp6.000\n`;
        simplemenu += `├ 30 Hari : Rp8.000\n\n`;
        simplemenu += `📝 SG-VIP ⚡ (2 IP / Unlimited Bandwidth)\n`;
        simplemenu += `🚀 CPU DO-Premium-AMD (4) @ 1.99GHz\n`;
        simplemenu += `├ 3 Hari : Rp3.000\n`;
        simplemenu += `├ 7 Hari : Rp6.000\n`;
        simplemenu += `├ 15 Hari : Rp9.000\n`;
        simplemenu += `├ 30 Hari : Rp10.000\n\n`;
        simplemenu += `📝 SG STB (5 IP / 400GB Bandwidth)\n`;
        simplemenu += `🚀 CPU DO-Standar-AMD (2) @ 1.49GHz\n`;
        simplemenu += `├ 30 Hari : Rp15.000\n`;
        simplemenu += `├ 60 Hari : Rp28.000\n\n`;
        simplemenu += `📝 SG VIP STB ⚡ (10 IP / Unlimited Bandwidth)\n`;
        simplemenu += `├ 30 Hari : Rp18.000\n`;
        simplemenu += `├ 60 Hari : Rp34.000\n\n`;
        simplemenu += `📝 INDO VIP NUSA (4 IP / Unlimited Bandwidth)\n`;
        simplemenu += `├ 30 Hari : Rp15.000\n\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
        simplemenu += `🎁 PROMO PAKET INJECT TERIMASSSSJADI\n\n`;
        simplemenu += `➥  XL VIDIO TANPA CONFIG❌ → Rp30.000\n`;
        simplemenu += `➥  XL VIDIO DENGAN CONFIG✅ → Rp40.000\n`;
        simplemenu += `├FUP: 100GB–150GB All App\n`;
        simplemenu += `├Tanpa gandeng\n\n`;
        simplemenu += `➥  XL IFLIX TANPA CONFIG❌→ Rp33.000\n`;
        simplemenu += `➥  XL IFLIX DENGAN CONFIG✅→ Rp43.000\n`;
        simplemenu += `├FUP: 100GB–150GB All App\n`;
        simplemenu += `├Tanpa gandeng\n\n`;
        simplemenu += `➥  Masa Aktif 1 Tahun → Rp10.000\n`;
        simplemenu += `├Nomor tidak boleh masa tenggang\n`;
        simplemenu += `├Jika tenggang → isi paket edukasi 2GB/1hari dulu\n`;
        simplemenu += `├Masa aktif bertambah setiap bulan sampai 1 tahun\n\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
        simplemenu += `📌 Syarat & Ketentuan\n\n`;
        simplemenu += `├ Tidak perlu pulsa\n`;
        simplemenu += `├ Estimasi masuk 1 – 30 menit\n`;
        simplemenu += `├ Wajib OTP\n`;
        simplemenu += `├ Bisa Inject sampai ratusan GB\n`;
        simplemenu += `├ Boleh tanya dulu jika bingung\n\n`;
        simplemenu += `⚠️ Notes:\n`;
        simplemenu += ` Garansi 100% masuk Dari XL\n`;
        simplemenu += `├ Membeli = Setuju\n`;
        simplemenu += `├ Paket ilegal bisa hilang (di hapus XL) → Tidak ada refund\n`;
        simplemenu += `(Selama ini aman 👍)\n\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n`;
        simplemenu += `⚡ Config Premium Server Singapura Untuk Injektor Kuota.\n`;
        simplemenu += `✅ FULL GARANSI → Jika EROR langsung ditangani sampai connect!\n\n`;
        simplemenu += `🌐 Online Order/Trial:\n`;
        simplemenu += `https://list.maourafavpn.my.id\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n`;
        simplemenu += `INDONESIA 🇮🇩 & SINGAPURA 🇸🇬\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
        simplemenu += `📞 *Order & Trial*\n`;
        simplemenu += `☎️𝗪𝗛𝗔𝗧𝗦𝗔𝗣𝗣\n`;
        simplemenu += `𝗢𝗥𝗗𝗘𝗥\n`;
        simplemenu += `➥ wa.me/+6282278687542\n`;
        simplemenu += `➥ wa.me/+6287895338730\n`;
        simplemenu += `█║▌║▌║ - ║▌║▌║\n`;
        simplemenu += `💳 𝗣𝗔𝗬𝗠𝗘𝗡𝗧\n`;
        simplemenu += `➥ *Dana* 087895338730\n`;
        simplemenu += `➥ *Qris* list.maourafavpn.my.id/qris.html\n`;
        simplemenu += `█║▌║▌║ - ║▌║▌║\n`;
        simplemenu += `🎬 Trial Config Mingguan\n`;
        simplemenu += `➥ https://youtu.be/@maourafa\n`;
        simplemenu += `━━━━━━━━━━━━━━━━━━━━━━━\n`;
        simplemenu += `${readmore}✨ Yuk gabung sekarang & nikmati layanan VPN Premium stabil, murah, full garansi dari MAOUVPN! 🚀`;
                ...
                "sections": [
                    {
                        "title": "📌 Menu Utama",
                        "rows": [
                            { "header": "📚 List Harga", "title": "Lihat semua fitur Harga!", "id": `${prefix}webvpn` },
                            { "header": "🗝️ Owner", "title": "Nomor Owner!", "id": `${prefix}owner` },
                            { "header": "👥 Saluran", "title": "Fitur khusus buat grup", "id": `${prefix}chwa` },
                        ]
                    }
                ]
            })
        ...
        switch (typemenu) {
            case "v1":
                ...
                break;
            case "v2":
                await m.reply(simplemenu, { contextInfo });
                break;
            case "v3":
                await sock.sendButtonImage(m.chat, { url: imageUrl }, button, simplemenu, footer, m);
                break;
            case "v4":
                await sock.sendButtonText(m.chat, button, simplemenu, footer, m);
                break;
            case "v5":
                await m.reply({ image: imageBuffer, caption: simplemenu });
                break;
            case "v6":
                await m.reply({ video: videoBuffer, caption: simplemenu });
                break;
            case "v7":
                await m.reply({ video: videoBuffer, gifPlayback: true, caption: simplemenu });
                break;
            case "v8":
                await newReply(simplemenu);
                break;
            case "v9":
            ...
            case "v14":
                ...
                break;
            default:
                await m.reply("⚠️ Menu belum diatur! Coba lagi nanti ya 😅");
                break;
        }
        
    } catch (error) {
        console.error('Error in config/vpn command:', error);
        await m.react('❌');
  ...
  await sock.sendMessage(m.chat, {
    text: `https://youtu.be/GACC2BMCDiw\n\n• AXIS EDUKASI
https://sfile.mobi/4jtd1KZxxuj
• AXIS GAME
https://sfile.mobi/4jtd1KZxxuj
• AXIS GAME 2
https://sfile.mobi/4jtd1KZxxuj
...
break; 
			case 'chwa':{
			  newReply(' list.maourafavpn.my.id')
			}
			break;
			case 'webvpn':{
			  newReply('list.maourafavpn.my.id')
